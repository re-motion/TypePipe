    ...
    public virtual Assembly TryLoadAssembly (AssemblyName assemblyName, string context)
    {
      ...
      if (_filter.ShouldConsiderAssembly (assemblyName))
      {
        ...
        catch (FileLoadException ex)
        {
          s_log.WarnFormat (
              ex,
              "Assembly {0} triggered FileLoadException - maybe a referenced assembly is missing? (Or the assembly could be DelaySigned.) "
              +  "The assembly was loaded in the following context: {1}.", assemblyName, context);
          return null;
        }
      }
      else
